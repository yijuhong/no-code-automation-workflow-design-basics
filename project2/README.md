...
| AI 요약 Action | Simple Text Prompt가 실행되고 Slack 공유용 요약문이 생성된다. | Simple Text Prompt가 실행되지 않는다. |
| Google Sheets 기록 | `결정사항 알림 기록` 탭에 회의명, 결정사항, 후속작업, 담당자, AI 요약문이 추가된다. | `회의록 보관 기록` 탭에 회의명, 회의일시, 참석자, 회의록 본문이 추가된다. |
